...
function TagChip({ tag, placed, draggableProps }) {
  ...
  return (
    <div
      ...
      className={`${baseCls} bg-white border-ink/15 text-ink shadow-pop cursor-grab active:cursor-grabbing hover:border-L2 hover:-translate-y-0.5 ${
        draggableProps?.isDragging ? 'opacity-0' : ''
      }`}
    >
  ...
  if (placedTag) {
    return (
      <motion.button
        ...
        disabled={locked}
        className="group inline-flex items-center gap-1.5 align-baseline mx-0.5 my-0.5 px-2.5 py-1 rounded-lg bg-L2/10 border-2 border-L2 text-L2 text-[13.5px] font-semibold hover:bg-L2/15 transition"
        title={locked ? '' : 'Click to remove'}
      >
        <span>{placedTag.emoji}</span>
        <span>{placedTag.short}</span>
  ...
  return (
    <span
      ...
      className={`inline-flex items-center align-baseline mx-0.5 px-3 py-1 rounded-lg text-[13.5px] font-mono transition-all ${
        isOver
          ? 'bg-L2/15 border-2 border-solid border-L2 text-L2 scale-110'
          : 'border-2 border-dashed border-line text-soft bg-white/60'
      }`}
    >
      [{slot.placeholder}]
    </span>
  ...
  function handleDragEnd(e) {
    ...
    setPlacements((prev) => {
      ...
      for (const slotId of Object.keys(next)) {
        if (next[slotId] === active.id) delete next[slotId];
      }
  ...
  return (
    <DndContext sensors={sensors} onDragStart={handleDragStart} onDragEnd={handleDragEnd}>
      <div className="flex items-center gap-3 mb-3">
        <div className="w-9 h-9 rounded-xl bg-L2 text-white flex items-center justify-center font-mono font-bold text-sm shadow-pop">02</div>
        <Eyebrow color="text-L2">Level 2 of 3 · The Context Anchor</Eyebrow>
      </div>

      <h2 className="font-display text-3xl sm:text-[40px] font-bold tracking-tight mb-3 leading-[1.05]">
        Vague prompts give junk. <span className="text-L2">Anchor it.</span>
      </h2>
      <p className="text-muted mb-7 leading-relaxed text-[15.5px] max-w-xl">
        Drag the three context tags into the prompt formula. Watch a useless
        email transform into a Singapore-ready tenant circular.
      </p>

      <div className="grid sm:grid-cols-2 gap-3 mb-4">
        <div className="card p-5 relative overflow-hidden">
          <div className="absolute top-0 left-0 right-0 h-0.5 bg-L3" />
          <div className="flex items-center gap-2 mb-3">
            <div className="px-2 py-0.5 rounded-md bg-L3/10 text-L3 text-[10.5px] font-bold uppercase tracking-[0.15em]">
              Vague
            </div>
          </div>
        ...
        <div className="card-strong p-5 relative overflow-hidden">
          <div className="absolute top-0 left-0 right-0 h-0.5 bg-L2" />
          <div className="flex items-center gap-2 mb-3">
            <div className="px-2 py-0.5 rounded-md bg-L2/10 text-L2 text-[10.5px] font-bold uppercase tracking-[0.15em]">
              Anchored
            </div>
          </div>
          <div className="text-[13.5px] bg-cream rounded-lg p-3 border border-line leading-[2.2]">
            ...
            <DropSlot
              slot={SLOTS[0]}
              placedTag={placements.audience && TAGS.find((t) => t.id === placements.audience)}
              onClear={clearSlot}
              locked={generated}
            />
            <span> in a </span>
            <DropSlot
              slot={SLOTS[1]}
              placedTag={placements.tone && TAGS.find((t) => t.id === placements.tone)}
              onClear={clearSlot}
              locked={generated}
            />
            ...
            <DropSlot
              slot={SLOTS[2]}
              placedTag={placements.details && TAGS.find((t) => t.id === placements.details)}
              onClear={clearSlot}
              locked={generated}
            />
            <span>.</span>
      ...
      <div className="card p-5 mb-5">
        <div className="text-[11px] font-semibold uppercase tracking-[0.22em] text-muted mb-3">
          🏷️ Drag these context tags into the prompt
        </div>
        <div className="flex flex-wrap gap-2.5">
          {TAGS.map((t) => (
            <Draggable
              key={t.id}
              tag={t}
              placed={Object.values(placements).includes(t.id)}
            />
          ))}
        </div>
      </div>
        ...
        {generated && (
          ...
          >
            <div className="card-strong overflow-hidden">
              <div className="flex items-center gap-2.5 px-5 py-3 border-b border-line bg-gradient-to-r from-L2/5 to-transparent">
                <div className="w-7 h-7 rounded-lg bg-L2 text-white flex items-center justify-center text-[11px] font-bold">AI</div>
                <div className="text-[11px] font-semibold uppercase tracking-[0.18em] text-L2">
                  Streaming output
                </div>
              ...
              <pre className="whitespace-pre-wrap font-sans text-[14.5px] leading-relaxed text-ink p-5 caret">
                {generatedText}
              </pre>
            </div>
          </motion.div>
      ...
      <div className="flex justify-between items-center flex-wrap gap-3">
        <button
          ...
          className={`px-5 py-3 rounded-xl font-semibold text-[15px] transition-all ${
            allPlaced && !generated
              ? 'bg-L2 text-white hover:bg-[#EA580C] shadow-pop ring-pulse-orange'
              : generated
              ? 'bg-win/10 text-win border border-win/30 cursor-default'
              : 'bg-line text-soft cursor-not-allowed'
          }`}
        >
          {generated ? '✓ Generated' : '⚡ Generate Tenant Circular'}
        </button>
        <PrimaryButton onClick={onComplete} disabled={!generated} accent="L2">
          Next Level →
        </PrimaryButton>
      </div>

      <Tip accent="L2">
        <span className="font-semibold text-ink">Why it matters:</span> Adding{' '}
        <span className="font-semibold text-ink">audience, tone, and details</span>{' '}
        turns a generic AI dump into something you can send with one quick review.
      </Tip>

      <DragOverlay dropAnimation={{ duration: 200, easing: 'cubic-bezier(0.22, 1, 0.36, 1)' }}>
        {activeTag ? (
          <div className="inline-flex items-center gap-2 px-3.5 py-2.5 rounded-xl text-[14px] font-semibold bg-white border-2 border-L2 text-ink shadow-ring rotate-[-2deg] cursor-grabbing">
            <span className="text-base">{activeTag.emoji}</span>
